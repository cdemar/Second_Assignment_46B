...
public class WeatherTester {
	...
	private static boolean testBasicClassesExist() {
		...
		String[] basicNames = { "Cloud", "CirrusCloud", "CumulusCloud",
				"StratusCloud", "Sky" };
	...
	public static boolean testSkyStructure() {
		try {
			...
			if (cloudsField.getType() != java.util.ArrayList.class) {
				System.out.println("Unexpected type for clouds in Sky: "
						+ cloudsField.getType() + " should be ArrayList");
				return false;
			}
	...
	public static boolean testCloudSubclassing() {
		...
		for (String subCloud : subClouds) {
			try {
				Class<?> superclass = Class.forName("weather." + subCloud)
						.getSuperclass();
		...
		if (!message.isEmpty())
			...
			System.out
					.println("Subclassing for all the cloud classes is correct");

		return message.isEmpty();
	}

	public static boolean testCloudCtors() {
		String[] allClouds = { "Cloud", "CirrusCloud", "CumulusCloud",
				"StratusCloud" };
		String message = "";
		Class<?>[] expectedArgTypes = { float.class, float.class };
		for (String cloud : allClouds) {
			try {
				Class.forName("weather." + cloud).getDeclaredConstructor(
						expectedArgTypes);
			} catch (ClassNotFoundException x) {
